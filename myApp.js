...
// as `MONGO_URI`. Connect to the database using `mongoose.connect(<Your URI>)`r** # SCHEMAS and MODELS #
/*  ====================== */
...
var queryChain = function(done) {
  ...
  var fullQuery = Person.find({ favoriteFoods: foodToSearch })
    .sort("name")
    .limit(2)
    .select("-age");

  fullQuery.exec(function(err, data) {
    if (err) return done.error(err);
    done(null, data);
  });
};
